...
- Terraform plan, apply, and destroy stay in separate GitHub Actions workflows on purpose. Plan is review and audit oriented; apply is a privileged deployment path with a confirmation gate; destroy is rarer and riskier, so it gets its own manual-only entry point. Apply and destroy use the GitHub Actions environment named `production` for approval/protection rules, but the Terraform target is still chosen separately through `infra_environment` / `TF_INFRA_ENVIRONMENT` and can point at `envs/dev` or `envs/prod`. Terraform Cloud and Atlantis would be reasonable upgrades for a team with shared operations, policy-as-code, or PR comment workflows, but they add a hosted service and more account setup than this sample needs. A single `workflow_dispatch` workflow with an action input would reduce YAML, but it would also put read-only planning, mutation, and teardown behind one button surface, which is a worse operator experience for a demo that is meant to make blast radius obvious.
