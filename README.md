...
- **Operator script tests:** 8 Python unit tests cover Terraform drift parsing/reporting and migration ordering, idempotent skips, and rollback behavior. [`scripts-lint.yml`](.github/workflows/scripts-lint.yml) runs those tests, Pylint, and a real PostgreSQL migration replay to verify cold-start and idempotent behavior. Docker CI starts the built API image and runs [`scripts/health_check.py`](scripts/health_check.py) against `localhost:3000`.
